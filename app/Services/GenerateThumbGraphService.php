...
class GenerateThumbGraphService
{
    ...
    public function generateGraph(){
        $graphArray = [];
        //dd($this->coinGraphData['usdp']);
        foreach ($this->coinGraphData as $id => $value) {
            //dd($this->coinGraphData[$id]);
            if(!empty($this->coinGraphData[$id])){
                $maxValue = max($this->coinGraphData[$id]);
                $minValue = min($this->coinGraphData[$id]);
                //$quantity = count($this->coinGraphData[$id]);
                ...
                $img = "<svg width='168' height='50' viewBox='0 0 168 50' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink'> <polyline xmlns='http://www.w3.org/2000/svg' points='' fill='none' stroke='' stroke-width='1.25'/></svg>";
            }
            $graphArray[$id] = $img;
            //printf($img);
            //dd(public_path());
